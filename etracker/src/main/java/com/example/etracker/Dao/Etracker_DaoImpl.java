...
public class Etracker_DaoImpl implements Etracker_Dao {
    ...
    public Collection<Map<String,java.lang.Object>> graph4(int userId,int year, int month) {
		...
		return jdbcTemplate.queryForList(Sql.MetadataSql.TOTAL_LINE_MONTH,year,month,userId);
	}
    public List<Map<String, Object>> monthlycategorysum(int userId,int month,int year) {
		return jdbcTemplate.queryForList(Sql.MetadataSql.CATEGORY_BAR_MONTH,userId,month,year);

	}

	public List<Map<String, Object>> yearlycategorysum(int userId,int year) {
		return jdbcTemplate.queryForList(Sql.MetadataSql.CATEGORY_BAR_YEAR,userId,year);
